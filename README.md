# UR3 Cloud-Based Digital Twin on AWS

This repository contains the Infrastructure as Code (IaC) and device-side edge software for a Cloud-Based Digital Twin of an industrial UR3 robotic arm. This project was developed as a BSc Thesis in Computer Science at Sapientia Hungarian University of Transylvania.

The goal of the system is to seamlessly connect a physical Universal Robots UR3 manipulator to the AWS Cloud, enabling real-time bidirectional telemetry monitoring, state synchronization, and remote command execution.

## Architecture Overview


The system is built on a modern, event-driven serverless architecture using AWS IoT Core as the central communication hub.

### Tech Stack
* **Cloud Provider:** Amazon Web Services (AWS)
* **Infrastructure as Code (IaC):** Terraform
* **IoT & Messaging:** AWS IoT Core (MQTT, Device Shadow), Amazon SQS, Amazon Kinesis
* **Compute & API:** AWS Lambda (Python), Amazon API Gateway, AWS AppSync
* **Edge Device:** Python (paho-mqtt) running on the local control network interacting with the UR-RTDE interface

### Core Features
* **Real-Time Telemetry:** The edge script continuously monitors joint positions and streams state changes to AWS IoT Core via MQTT with smart compression.
* **Device Shadow Integration:** Maintains a reliable, asynchronous cloud representation of the robot's state, allowing applications to query the last known position even if the physical robot is offline.
* **Bidirectional Control:** Secure, authenticated remote execution of movement commands (`moveJ`, `moveL`) or emergency stops routed from the cloud back to the physical controller.
* **Serverless Backend:** Highly scalable, on-demand infrastructure deployed automatically via CI/CD pipelines.
...
* **IAM Role for GitHub Actions:** A dedicated IAM role that allows the GitHub Actions pipeline to securely access the AWS account without keys using OIDC.
...
Ez a tároló egy ipari UR3 robotkar felhőalapú digitális ikertestvérének infrastruktúra kódját (IaC) és edge oldali szoftverét tartalmazza. A projekt a Sapientia Erdélyi Magyar Tudományegyetem Marosvásárhelyi karán készült BSc szakdolgozatként.

A rendszer célja egy fizikai Universal Robots UR3 manipulátor zökkenőmentes csatlakoztatása az AWS felhőhöz, amely lehetővé teszi a valós idejű, kétirányú telemetria megfigyelést, az állapotszinkronizációt és a távoli parancsvégrehajtást.

## Architektúra Áttekintés


A rendszer egy modern, eseményvezérelt serverless architektúrára épül, amelynek központi kommunikációs csomópontja az AWS IoT Core.

### Felhasznált Technológiák (Tech Stack)
* **Felhőszolgáltató:** Amazon Web Services (AWS)
* **Infrastruktúra kódként (IaC):** Terraform
* **IoT és Üzenetküldés:** AWS IoT Core (MQTT, Device Shadow), Amazon SQS, Amazon Kinesis
* **Szerver nélküli futtatás és API:** AWS Lambda (Python), Amazon API Gateway, AWS AppSync
* **Edge Eszköz (Kliens):** Python (paho-mqtt) a helyi hálózaton, amely az UR-RTDE interfészen keresztül kommunikál a robottal.

### Fő Funkciók
* **Valós idejű telemetria:** Az edge szkript folyamatosan figyeli a csuklópozíciókat, és intelligens tömörítéssel, MQTT-n keresztül továbbítja az állapotváltozásokat az AWS IoT Core felé.
* **Device Shadow Integráció:** Fenntartja a robot állapotának megbízható, aszinkron felhős reprezentációját, így az alkalmazások akkor is lekérdezhetik az utolsó ismert pozíciót, ha a fizikai robot offline állapotban van.
* **Kétirányú vezérlés:** Mozgási parancsok (`moveJ`, `moveL`) vagy vészleállítások biztonságos, hitelesített távoli végrehajtása a felhőből a fizikai vezérlőhöz irányítva.
* **Serverless Backend:** Magasan skálázható, igényalapú infrastruktúra, amely CI/CD folyamatokon keresztül automatikusan települ.
...
- A robot IP címe elérhető kell legyen az edge eszközről.
...
* **IAM Role for GitHub Actions:** Egy dedikált IAM szerepkör, ami OIDC hitelesítéssel lehetővé teszi, hogy a GitHub Actions pipeline biztonságosan, statikus kulcsok nélkül hozzáférjen az AWS fiókhoz.
...
3. **Hozza létre az erőforrásokat:**
    Futtassa az `apply` parancsot. A Terraform megtervezi és a jóváhagyását kéri az erőforrások létrehozásához. Írjon be `yes`-t a megerősítéshez.
    ```bash
    terraform apply
    ```
